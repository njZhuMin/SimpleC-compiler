 ...
 * 解决冲突的方法：解决冲突的方法是分析所有含A和B的句型，考察集合FOLLOW(A)和FOLLOW(B)，
 * 如果这两个集合不相交，而且也不包含b，那么当状态I面临输入符号a时，我们可以使用如下策略：
 *
 * 若a=b，则移进。
 ...
 */
/* SLR的基本算法：
 * 假定LR(0)规范族的一个项目集I中含有m个移进项目 A1→α•a1β1，A2→α•a2β2，…，Am→α•amβm；
 * 同时含有n个归约项目 B1→α•，B2→α•，…，B3→α•，
 * 如果集合{ a1,…, am}，FOLLOW(B1)，…，FOLLOW(Bn)两两不相交（包括不得有两个FOLLOW集合有#），
 * 则隐含在I中的动作冲突可以通过检查现行输入符号a属于上述n+1个集合中的哪个集合而活的解决：
 *
 * 若a是某个ai，i=1,2,…,m，则移进。
 ...
 * 若项目A→α•属于Ik，那么，对任何非终结符a，a∈FOLLOW(A)，
 * 置ACTION[k,a]为“用产生式A→α进行归约”，简记为“rj”；其中，假定A→α为文法G’的第j个产生式
...
struct Content {
    ...
    Content(int a, int b) : type(a), num(b) {}
};
...
void make_item() {
    ...
    for(int i = 0 ; i < wf.size(); i++)
        for(int j = 0 ; j <= wf[i].right.length(); j++) {
            ...
            items.push_back(WF(wf[i].left, temp, i, (int)items.size()));
        }
    #ifdef DEBUG
    puts("-------------------------项目表-------------------------");
...
void analyse(string src) {
    ...
    for(int i = 0 ; i < src.length(); i++) {
        ...
        if(act.type == 0) {
            print(get_steps(steps++), get_stk(op_stack), src.substr(i), "shift",
                  get_stk(st_stack), act.out, "");
            ...
            print(get_steps(steps++), get_stk(op_stack), src.substr(i), get_shift(tt),
                  get_stk(st_stack),act.out,get_steps(x));
...
int main(int argc, char ** argv) {
    ...
    string rule;
    if (argc != 2) {
        cout << "Usage: ./SLR1 grammar_file." << endl;
        return -1;
    }
    ...
    cout << endl << "Input rule to recognize:" << endl;
    rule.resize(20);
    scanf("%s", &rule[0]);
    analyse(rule);
}
